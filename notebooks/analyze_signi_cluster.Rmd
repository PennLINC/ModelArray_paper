---
title: "Analyze and plot the GAM results for significant fixel cluster"
author: "Chenying Zhao"
output: html_document
---

This script is to analyze the fixel cluster of significance. \
Here, python file = ModelArray_paper/notebooks/analyze_signi_cluster.py \
Steps: \
0. Preparation \
    - Convert results from .h5 to .mif \
    - View in mrview, set a threshold, then draw an ROI as mask, save it \
    - example filename: ROI_x69_sage_p_lt_1e-15.mif 
  
1. [python] function: convert_voxelMask_to_fixelIndex()
    - This will generate the list of fixel's ids that included in manually drawn mask (ROI.mif). This also includes those fixels that do not meet thresholding criteria (although the filename might be misleading to contain e.g. "lt_1e-15")
    - example filename: ROI_x69_sage_p_lt_1e-15_fixelIdList.txt
...
2. [R] All significant fixels: the list 
    ...
    - example filename: gam_allOutputs_s_Age.p.value_lt_1e-15_fixelIdList.txt
    
3. [R] take the intersect of 1 and 2; save it.
    - example filename: gam_allOutputs_s_Age.p.value_lt_1e-15__Intersect__ROI_x69_sage_p_lt_1e-15_fixelIdList.txt
...
4. [python] function: save_selectedFixel_mask()
    ...
    - example filename: gam_allOutputs_s_Age.p.value_lt_1e-15__Intersect__ROI_x69_sage_p_lt_1e-15_fixelIdList.mif
  
5. [R] Analyze the intersect
    - make sure that all fixels selected meet the thresholding criteria (expect_true())
    - average the values across all fixels in this cluster, re-fit GAM, print out statistics, and generate the figure.
...
```{r install and load ModelArray and source necessary functions}
...
source("GAMM_plotting.R")   # Bart Larsen's function for visualizing gam results
source("utils.R")   # source partialRsq()
```

```{r functions for steps, echo=FALSE}
...
## Input arguments, PLEASE REVIEW AND CHANGE AS NEEDED!
```{r input arguments}
num.subj <- 938
fn.h5.results <- paste0("../../data/data_from_josiane/results/ltn_FDC_n",toString(num.subj),"_wResults_nfixels-0_20220109-183909.h5")
...
#print(paste0("object size of modelarray = ", object.size(modelarray)/1024, " kilobytes"))   # default object.size() is bytes
...
## Step 3: Getting the intersection
```{r step 3, echo=FALSE}
...
```{r step 5.1 average, echo=FALSE}
# load the final list:
fixel_id_list_intersect <- scan(fn.fixel_id_list_intersect, what="", sep="\n") %>% as.integer()
message("the final list of fixel ids selected is in file:")
message(fn.fixel_id_list_intersect)
message("number of fixels selected (i.e. within the ROI & meeting the threshold criteria:")
length(fixel_id_list_intersect)
...
message("For the fixels in this cluster (i.e. fixels meeting thresholding criteria):")
message("max raw p.value: ")
dat_selectedFixels_metric$s_Age_p.value %>% max()
message("max p.value after bonferroni:")
dat_selectedFixels_metric$s_Age_p.value %>% max() * num_fixel_total
message("number of fixels:")
numElementsTotal(modelarray, scalar_name = scalar_name)
...
```{r step 5.2 functions for plots, echo=FALSE}
...
plot_oneFixel <- function(modelarray, fixel_id, scalar_name,
  ...
  temp <- visualize_model(onemodel, smooth_var = 'Age')
  f <- temp$final_plot
  pred <- temp$pred   # the predicted values for drawing the fitted lines and se ranges
...
```{r step 5.3 generate the plots, echo=FALSE}
# plot one fixel:
...
message("summary for GAM for averaged values of this cluster:")
...
```{r step 5.4 save figures, echo=FALSE}
### save the figure:
final_width <- 90
final_height <- 85
