...
# NOTES!!! at present this file cannot be run in conda env, i.e. ggpattern hasn't been properly installed, as error when installing "units" package. See here: https://github.com/r-quantities/units/issues/223
...
plotGen_compare_diffncores <- function(totals_modelarray, totals_mrtrix,
  ...
  fontsize_text <- 3
  fontsize_theme <- 9
  theme.legend.key.size_value <- 3.5   # for plots made by ggpattern, this value is a bit smaller than plots by ggplots
  theme.legend.key.size_unit <- "mm"
  theme.legend.spacing.x_value <- 1
  theme.legend.spacing.x_unit <- "mm"
  # tried *.y but does not apply to ggpattern - there is no gap between legends squares in ggpattern...
  
  f <- ggplot(together_totals, aes(fill=software, y=max.total.GB., x=nparallel)) + 
              ggpattern::geom_bar_pattern(position = position_dodge(width = 0.5), stat="identity", width=0.5,    # position="dodge"
                                          #color="black",size=0.1,   # outline of the bars
                                          aes(pattern_fill2=software),
                                          pattern="gradient", pattern_fill=I(c("#CFCFCF"))) +    # , pattern_fill2=I(c(color_mrtrix, color_modelarray))
                # ggpattern::geom_bar_pattern(position = position_dodge(width = 0.5), stat="identity", width=0.5,    # position="dodge"
                #                             color="black",size=0.1,
                #                             pattern_color="white", pattern_fill="white", pattern="stripe",
                #                             pattern_spacing=0.03, pattern_density=0.02) +    
              ggpattern::scale_pattern_fill2_manual(values= c(color_mrtrix, color_modelarray) ) +
...
plotGen_compare_diffsubj <- function(totals_modelarray, totals_mrtrix, ncpus, 
  ...
  f <- ggplot(together_totals, aes(fill=software, y=max.total.GB., x=nsubj)) + 
                ggpattern::geom_bar_pattern(position="dodge", stat="identity",    # in original geom_bar()
                                          #color="black",size=0.1,   # outline of the bars
                                          aes(pattern_fill2=software),
                                          pattern="gradient", pattern_fill=I(c("#CFCFCF"))) +    # , pattern_fill2=I(c(color_mrtrix, color_modelarray))
...
color_mrtrix <- "#707070"   # #909090 will be lighter gray
color_modelarray <- "#248B46"
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

if (flag_where_now == "vmware") {
  main.folder.modelarray <- "/home/chenying/Desktop/fixel_project/FixelArray_benchmark" # TODO: change to ModelArray_benchmark
  main.folder.mrtrix <- "/home/chenying/Desktop/fixel_project/data/data_from_josiane/FDC/stats_FDC"
...
foldernames.list.mrtrix <- sort_foldernames(foldernames.list.mrtrix, "mrtrix","ncpus")
folders.list.mrtrix <- file.path(main.folder.mrtrix,  foldernames.list.mrtrix)
...
foldernames.list.mrtrix <- sort_foldernames(foldernames.list.mrtrix, "mrtrix", "nsubj")
folders.list.mrtrix <- file.path(main.folder.mrtrix,  foldernames.list.mrtrix)
foldernames.list.mrtrix
