...
plotGen_modelarray_diffncores <- function(clean.summary.when.max_modelarray, 
                                    ...
                                    delta_y = 0.15, ylim = c(0,10), flag_ylab = TRUE) {
  fontsize_text <- 3
  fontsize_theme <- 9
...
plotGen_compare_diffncores <- function(totals_modelarray, totals_mrtrix,
                                       ...
                                       delta_y = 0.5, ylim = c(0,52), flag_ylab = TRUE) {
  ...
  f <- ggplot(together_totals, aes(fill=software, y=max.total.GB., x=nparallel)) + 
                geom_bar(position = position_dodge(width = 0.6), stat="identity", width=0.6) +     # position="dodge"
...
plotGen_modelarray_diffsubj <- function(clean.summary.when.max_modelarray, totals_modelarray, 
  ...
  f <- ggplot(clean.summary.when.max_modelarray, aes(fill=process, y=memory_GB, x=nsubj)) + 
                ...
                geom_text(aes(x = nsubj, y = total.RSS.GB.+delta_y, label = sprintf("%0.2f", round(total.RSS.GB., digits = 2)), fill = NULL),
                          data = totals_modelarray, size=fontsize_text, angle = 30, family = "Arial") +   # , hjust = 0, nudge_x = -0.05, vjust = 0
...
plotGen_compare_diffsubj <- function(totals_modelarray, totals_mrtrix, ncpus, 
                                     color_mrtrix, color_modelarray, 
                                     delta_y=2.5, ylim = c(0,52), angle_text = 30, theme.legend.position = "right", flag_ylab = TRUE) {
  # adjust a bit before combination:
  totals_modelarray_ready <- totals_modelarray
  totals_modelarray_ready <- rename(totals_modelarray_ready, nparallel = ncore)   # new.col.name = old.col.name
  totals_modelarray_ready <- rename(totals_modelarray_ready, ModelArray = total.RSS.GB.) 
...
f_modelarray[[2]] <- plotGen_modelarray_diffncores(clean.summary.when.max_modelarray, totals_modelarray, 
                                              nsubj = nsubj, roof.num.child = roof.num.child,
                                              flag_legend_order_reverse, color_parent_process, color_children_process,
                                              ylim = ylim_modelarray, flag_ylab = FALSE)
...
f_compare[[2]] <- plotGen_compare_diffncores(totals_modelarray, totals_mrtrix,
                                       ...
                                       color_mrtrix, color_modelarray,
                                       delta_y = 2.5,
                                       ylim = ylim_compare, flag_ylab = FALSE)
...
p_together <- f_modelarray[[1]]  + f_modelarray[[2]] + f_compare[[1]] + f_compare[[2]] + plot_layout(nrow=2, heights = unit(c(50,50), c('mm','mm')), widths = unit(c(48, 48), c('mm','mm')), guides = 'collect') & theme(legend.position = 'right')  # , legend.justification = "left"
...
f_compare <- plotGen_compare_diffsubj(totals_modelarray, totals_mrtrix, ncpus, 
                                       color_mrtrix, color_modelarray, 
                                       delta_y=2.5, ylim= ylim_compare, angle_text = 0, theme.legend.position = c(0.2,0.8),
                                      flag_ylab = TRUE)
f_compare
...
#   area(2, 1, 2, 5)
# )
# p_together <- f_modelarray + guide_area()   + f_compare + plot_layout(design = layout, nrow=2) # & theme(legend.position = 'right')

layout <- "\n#AB\nCCC\n"
p_together <- f_modelarray + guide_area()   + f_compare + plot_layout(design = layout, nrow=2, heights = unit(c(50,50), c('mm','mm')), widths = unit(c(10, 60,1), c('mm'))) # & theme(legend.position = 'right')
final_width <- 140
final_height <- 140

# p_together <- f_modelarray+ f_compare + plot_layout(nrow=2, heights = unit(c(50,50), c('mm','mm')), widths = unit(c(80), c('mm')), 
